...
// --- Diffing ---
export function diffTrees(left, right) {
    const entries = [];
...
export function formatText(result, options = {}) {
    ...
    for (const entry of result.entries) {
        if (entry.changeType === "unchanged" && !showUnchanged)
            continue;
    ...
    if (s.added)
        lines.push(`  ${green}+ ${s.added} added${reset}`);
    if (s.removed)
        lines.push(`  ${red}- ${s.removed} removed${reset}`);
    if (s.modified)
        lines.push(`  ${yellow}~ ${s.modified} modified${reset}`);
    if (s.typeChanged)
        lines.push(`  ${cyan}! ${s.typeChanged} type changed${reset}`);
    if (s.unchanged)
        lines.push(`  ${dim}  ${s.unchanged} unchanged${reset}`);
